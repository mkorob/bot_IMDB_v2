...
2. If you use the `webpack` plugin, Size Limit will bundle your JS files into
   a single file. It is important to track dependencies andÿwebpack polyfills.
...
5. Then the `time` plugin runs headless Chrome (or desktop Chrome if it's
   available) toÿtrack the time a browser takes toÿcompile and execute your JS.
...
6. If you don't have a continuous integration service running, don't forget
   to add one -ÿstart with [Travis CI].
...
the Big Libraries preset above. [NanoÿID]ÿorÿ[Storeon]ÿareÿgoodÿexamples
for this preset.
...
6. Now, let's set the limit. Determine the current size of your library,
   add just a little bit (a kilobyte, maybe) andÿuseÿthatÿasÿtheÿlimit
...
9. Add the library size to docs, it will help users to choose your project:
    ...
    ```diff
      ...
      * **Fast.** 10% faster than competitor.
    + * **Small.** 500 bytes (minified and gzipped). Noÿdependencies.
    +   [Size Limit](https://github.com/ai/size-limit) controls the size.
    ```
...
[NanoÿID]: https://github.com/ai/nanoid/
...
* **path**: relative paths to files. The only mandatory option.
  ...
  orÿanÿarray `["index.js", "dist/app-*.js", "!dist/app-exclude.js"]`.
